...
LOOKBACK_TIME = 2 * 60 * 60  # ✅ Tikriname postus iš paskutinių 2 valandų
...
async def create_rss():
    ...
    min_time = datetime.datetime.utcnow() - datetime.timedelta(seconds=LOOKBACK_TIME)
    ...
    for msg, text in valid_posts:
        ...
        if msg.media:
            try:
                ...
                # ✅ Pakeista dalis – vietoj blob.exists()
                try:
                    blob.reload()  # Priverstinai patikriname failo egzistavimą
                except Exception as e:
                    logger.error(f"❌ Nepavyko užkrauti failo {blob_name}: {e}")
                    continue

                if not blob.exists():  # Patikriname dar kartą
